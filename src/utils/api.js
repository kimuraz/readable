...
const headers = { Authorization: 'whatever-i-want' };

const getEndpoint = (endpoint) => fetch(`${apiURL}/${endpoint}`, { headers }).then(res => res.json());
const postPutEndpoint = (endpoint, data, method = 'POST') => fetch(`${apiURL}/${endpoint}`, { method, headers, body: JSON.stringify(data) });
const deleteEndpoint = (endpoint) => fetch(`${apiURL}/${endpoint}`, { method: 'DELETE', headers});
